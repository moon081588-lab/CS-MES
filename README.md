...
`CKP-Watch.bat` 을 더블클릭해 창을 켜 두면 끝입니다. 그 창이 `_ckp_queue/` 폴더를
지켜보다가 Claude 가 넣은 요청을 받아 `run_all.py` 를 돌리고 결과를 돌립니다.
...
**왜 이게 필요한가요?.** MCP 로 등록하면 Claude 데스크톱 '대화' 에서는 도구가 보이지만,
Cowork 작업이 클라우드에서 실행되면 그 세션은 이 PC 의 MCP 프로세스를 볼 수 없습니다.
등록이 잘못된 게 아니라 세션이 다른 컴퓨터에 있어서 입니다. 폴더는 어느 쪽에서도 열리므로
이 방식은 클라우드·로컬·휴대폰 어디서 요청해도 똑같이 동작합니다.

**설치.** 저장소를 받으면 이미 들어 있습니다. 추가 설치 없음.
처음 한 번만 `CKP.bat` 의 2 번(처음 설정)으로 파이썬·openpyxl·지갑을 맞춰 두면 됩니다.
...
창을 두 번 켜면 두 번째는 스스로 물러납니다. 창이 죽으면 `.bat` 이 5 초 뒤 다시 띄웁니다.
