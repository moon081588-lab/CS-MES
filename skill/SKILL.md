---
name: CKP-skills-v12
description: CSI GMES MES(OCI 스키마)의 5개 핵심 화면 — 생산 실적, 부족분 관리, 수불 관리(P-O/O-I Cross Check), 품질 관리(OS&D Balance + Return Rate) — 와 근태·인사(Time & Labor, CKP)를 분석하는 skill. 트리거 키워드 — "Production Status", "생산 실적", "가동률", "라인별 % 컬럼", "Shortage Balance", "부족분", "남아있는 수량", "P-O Cross Check", "O-I Cross Check", "수불 관리", "출고-입고 정합성", "Out Only", "In Only", "External OS&D", "Internal OS&D", "후보충", "OS&D 발생", "Return Rate", "회수율", "Replenishment Rate", "C.GRADE RETURN", "CKP 리포트", "레포트", "보고서", "11개 리포트", "리포트/레포트 만들어줘", "뽑아줘", "작성해줘", 그리고 근태 — "근태", "출결", "출근율", "결근", "지각", "조퇴", "휴가", "교대/시프트", "사무직/NON_SHIFT", "Direct/Indirect/Overhead", "부서/직무별 근태". 분석은 이 4개 workflow(5개 화면) + 워크포스(PW_TL_DAILY_T·PW_HR_ACTION_T·PW_HR_EMPLOYEES) 범위 안에서만 수행하고, 임의 테이블 자유 분석은 하지 않는다. semantic layer YAML(2층 구조)이 진실의 출처. v12.7 — No.3 품목범위는 II 만(원본 대조 확정), 필터 출처는 core/sql.py, 설치 절차 안내, 기준 타임존 KST, DB 연결 환경 비의존화(도구는 접미사로 식별, 연결은 목록을 읽어 매칭, 접속 직후 OCI 검증 1회), 지갑/TNS_ADMIN 절차와 ORA 조치표 신설, 공식 11개 엑셀은 ckp-reports MCP 에 위임(workflows/reports-pipeline.md).
---
...
> **변경 이력**
> - **v12.7 (현재)** — **No.3 품목범위 확정**. 원본 수기 리포트(`CKP Manual Report (종합).xlsx` 의 `3-2. Balance IP Prod. by size` 시트, 기준 2026-04-20)의 Item Class 열이 II01~II93 여덟 종뿐이고 IP 는 257행 중 0행 → No.3 = `["II"]`. 같은 워크북 No.4 는 IP 가 대부분이므로 **비대칭은 원본 그대로**다. 코드(`core/sql.py`)도 함께 고쳤고, 실측(창 20260706~20260725) II 3,104 / IP 1,558 이므로 IP 분 1,558 이 빠진다. 더불어 새 PC 설치 절차를 `setup.bat` 한 번(계정·지갑 비밀번호 입력, 지갑 자동탐색, 방화벽 사전점검)과 `connect_claude.bat`(MCP 자동 등록) 기준으로 교체.
> - **v12.6** — 프로그램 통합에 맞춘 정정. ① 리포트별 필터의 진실의 출처가 `ckp_reports/core/sql.py` 의 `REPORTS` 로 바뀌었다(옛 `balance_sql.py` 참조 제거). ② **중복 제거 방식 변경 반영** — `END_ROUTING_YN` 대신 `(PCARD_NAME,ITEM_CLASS,SIZE_CD)` 별 `ROUTING_SEQ` 최대 1행(검증 CP 38,289→23,255). 옛 방식 숫자와 다른 것이 정상. ③ MCP 도구 목록을 실제와 맞춤(`ckp_status`·`list_report_dir`·`tail_ckp_log`·`ckp_reset`). ④ 마감필터는 프로그램이 커버리지를 재서 **자동 판정**(2026-07-28 기준 0% → loose). ⑤ 설치 안내를 새 전달 폴더(`CKP-Report` + run/setup/check.bat) 기준으로 교체. ⑥ No.3 품목범위(II vs II+IP)는 **미확정**으로 명시 — 원본 대조 전까지 단정 금지.
> - **v12.5** — 현지 Windows PC 에서 "레포트 11개 만들어줘" 한마디로 동작시키는 것이 목표. ① `workflows/reports-pipeline.md` §0 신설 — **도구(`…ckp_make_all`)가 없으면 SQL 로 흉내내지 말고** Windows 설치 절차(공백·한글 없는 경로, 지갑 위치, `install_windows.bat`, Desktop 완전 재시작)를 안내하고 멈춘다. '엑셀 11개'와 '숫자·분석' 요청을 구분하도록 명시. ② §2-1 신설 — 산출물은 채팅이 아니라 PC 폴더에 저장되므로 경로와 파일 목록을 답변에 적는다. ③ 트리거 키워드에 '레포트'(ㅔ 철자)·'보고서'·'만들어줘/뽑아줘/작성해줘' 추가.
> - **v12.4** — 기준 타임존을 **한국시간(Asia/Seoul, KST)** 으로 통일(조직 표준). §4-1 을 KST 기준으로 다시 쓰고, DB 날짜 컬럼은 현장(WIB, UTC+7) 시계라 **KST 00:00~01:59 구간에서만** 하루 어긋난다는 점(2시간 차)을 명시. §0-3 검증 쿼리에 KST_NOW 추가. 실무 디폴트가 '실데이터 최신일' 인 것과 `MAX(FA_DATE)` 에 미래 계획일이 섞이는 함정은 그대로 유지.
> - **v12.3** — 실측 기반 보강. ① `_common.md` §0-3 접속검증 쿼리에 **DB 타임존·현장(WIB) 시각·데이터 신선도**(`MAX(SCAN_YMD)`·`MAX(UPDATE_DT)`) 추가 — OCI 는 복제본이라 며칠 밀릴 수 있고, 그걸 모르면 빈 결과를 실제 0 으로 오해한다. ② §0-3-1 **재접속 규칙** 신설 — SQLcl MCP 연결은 유휴 시 끊기므로 `Connection not established`/ORA-03113 이면 조용히 재접속 후 재실행. ③ §4-1 전면 개편 — `SYSDATE`(UTC)·`CURRENT_DATE`(세션TZ)·현장(WIB, UTC+7)이 **서로 다른 '오늘'**이고, 현장 00:00~07:00(야간 3교대) 구간에서 `TRUNC(SYSDATE)` 가 전일을 가리키는 함정. 달력 기준이 필요하면 `TRUNC(SYSTIMESTAMP AT TIME ZONE 'Asia/Jakarta')` 사용, 단 실무 디폴트는 여전히 실데이터 최신일. `MAX(FA_DATE)` 는 미래 계획일 때문에 신선도 판단에 쓰면 안 된다는 점도 명시.
> - **v12.2** — 환경 비의존화 + 프로그램 연동. ① `workflows/_common.md` §0 전면 개편: MCP 도구를 **접두어가 아닌 접미사로 식별**(`mcp__remote-devices__sqlcl__*` / `mcp__sqlcl__*` 양쪽), 연결 이름 하드코딩 제거 후 **목록을 읽어 매칭**(순위: changshinincaipoc > 포함 > 접속문자열 > `*_medium` > `csi*`; `csi*` 는 비번 미저장 연결 때문에 마지막), `lmes` 계열 제외. ② **접속 직후 검증 1회 필수**(`ALL_TABLES` `OWNER='OCI'` 개수 + `SYSTIMESTAMP` 타임존). ③ **지갑/TNS_ADMIN 절차 신설** — 복사해 온 지갑의 `sqlnet.ora DIRECTORY` 함정, 경로에 공백·한글 금지(ORA-17956), Windows 절차. ④ **증상별 조치표**(ORA-12154/12263/17956/28759/29024/01017/00942/03113). ⑤ `workflows/reports-pipeline.md` 신설 — 공식 11개 엑셀은 SQL 로 재현하지 말고 `ckp-reports` MCP(`ckp_make_all`/`ckp_status`)에 위임, 60초 타임아웃 오해 방지, 리포트별 필터의 진실의 출처(`balance_sql.py` REPORTS)와 마감필터 차이 명시.
...
| `workflows/reports-pipeline.md` | **CKP 공식 11개 엑셀 리포트 생성·발송**(ckp-reports MCP) 위임 규칙 | "리포트 만들어줘/뽑아줘/보내줘" 요청 시 |
