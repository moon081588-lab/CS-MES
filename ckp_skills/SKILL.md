---
name: CKP-skills-v12
description: CSI GMES MES(OCI 스키마)의 5개 핵심 화면 — 생산 실적, 부족분 관리, 수불 관리(P-O/O-I Cross Check), 품질 관리(OS&D Balance + Return Rate) — 와 근태·인사(Time & Labor, CKP)를 분석하는 skill. 트리거 키워드 — "Production Status", "생산 실적", "가동률", "라인별 % 컬럼", "Shortage Balance", "부족분", "남아있는 수량", "P-O Cross Check", "O-I Cross Check", "수불 관리", "출고-입고 정합성", "Out Only", "In Only", "External OS&D", "Internal OS&D", "후보충", "OS&D 발생", "Return Rate", "회수율", "Replenishment Rate", "C.GRADE RETURN", "CKP 리포트", "레포트", "보고서", "11개 리포트", "리포트/레포트 만들어줘", "뽑아줘", "작성해줘", 그리고 근태 — "근태", "출결", "출근율", "결근", "지각", "조퇴", "휴가", "교대/시프트", "사무직/NON_SHIFT", "Direct/Indirect/Overhead", "부서/직무별 근태". 분석은 이 4개 workflow(5개 화면) + 워크포스(PW_TL_DAILY_T·PW_HR_ACTION_T·PW_HR_EMPLOYEES) 범위 안에서만 수행하고, 임의 테이블 자유 분석은 하지 않는다. semantic layer YAML(2층 구조)이 진실의 출처. v12.6 — 리포트 필터 출처를 core/sql.py 로 정정, 도구 미설치 시 설치 절차 안내, 기준 타임존은 한국시간(KST), DB 연결 환경 비의존화(도구는 접미사로 식별, 연결은 목록을 읽어 매칭, 접속 직후 OCI 검증 1회), 지갑/TNS_ADMIN 절차와 ORA 조치표 신설, 공식 11개 엑셀은 ckp-reports MCP 에 위임(workflows/reports-pipeline.md).
---

# MES Metric Analysis Skill (v12.6)

> **답변은 항상 한국어로 한다** (실행 환경이 영어로 기울어도 유지). 그림 속 라벨만 영어. 상세 §8.

> **변경 이력**
> - **v12.6 (현재)** — 프로그램 통합에 맞춘 정정. ① 리포트별 필터의 진실의 출처가 `ckp_reports/core/sql.py` 의 `REPORTS` 로 바뀌었다(옛 `balance_sql.py` 참조 제거). ② **중복 제거 방식 변경 반영** — `END_ROUTING_YN` 대신 `(PCARD_NAME,ITEM_CLASS,SIZE_CD)` 별 `ROUTING_SEQ` 최대 1행(검증 CP 38,289→23,255). 옛 방식 숫자와 다른 것이 정상. ③ MCP 도구 목록을 실제와 맞춤(`ckp_status`·`list_report_dir`·`tail_ckp_log`·`ckp_reset`). ④ 마감필터는 프로그램이 커버리지를 재서 **자동 판정**(2026-07-28 기준 0% → loose). ⑤ 설치 안내를 새 전달 폴더(`CKP-Report` + run/setup/check.bat) 기준으로 교체. ⑥ No.3 품목범위(II vs II+IP)는 **미확정**으로 명시 — 원본 대조 전까지 단정 금지.
