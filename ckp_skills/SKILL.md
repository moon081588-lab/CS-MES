---
name: CKP-skills-v12
description: CSI GMES MES 데이터베이스(OCI 스키마)의 5개 핵심 화면 — 생산 실적, 부족분 관리, 수불 관리(P-O/O-I Cross Check), 품질 관리(OS&D Balance + Return Rate) — 와 근태(Time & Labor, CKP)를 분석하는 skill. 트리거 키워드 — "Production Status", "생산 실적", "가동률", "라인별 % 컬럼", "Shortage Balance", "부족분", "남아있는 수량", "P-O Cross Check", "O-I Cross Check", "수불 관리", "출고-입고 정합성", "Out Only", "In Only", "External OS&D", "후보충", "OS&D 발생", "Damage/Lost/Issue/Shortage", "Return Rate", "회수율", "Replenishment Rate", "C.GRADE RETURN", 그리고 근태 — "근태", "출결", "출근율", "결근", "지각", "조퇴", "휴가", "교대/시프트", "사무직/NON_SHIFT", "Direct/Indirect/Overhead", "부서/직무별 근태". 분석은 이 4개 workflow(5개 화면) + 워크포스(근태·인사 — PW_TL_DAILY_T·PW_HR_ACTION_T·PW_HR_EMPLOYEES 등 정의된 PW 세트) 범위 안에서만 수행하며, 그 밖의 임의 테이블 자유 분석은 하지 않는다. 매칭 안 되는 질문은 무조건 가장 가까운 도메인으로 유도하고, 영 안 되면 범위 밖임을 안내한다. 분석 결과를 일보·브리핑·리포트 형태로 가공 가능. semantic layer YAML(2-layer 구조)을 진실의 출처로 사용해 SQL 작성. v12.2 변경점 — DB 연결 절차를 환경 비의존화(도구는 접미사로 식별, 연결 이름은 목록을 읽어 매칭, 접속 직후 OCI 테이블 검증 1회 필수), 지갑/TNS_ADMIN 절차와 ORA 증상별 조치표 신설, CKP 공식 11개 엑셀 리포트는 SQL 재현 대신 ckp-reports MCP 에 위임(workflows/reports-pipeline.md). 워크포스 도메인은 근태+인사(PW_TL_DAILY_T·PW_HR_ACTION_T·PW_HR_EMPLOYEES, PII 3단계 규칙).
---

# MES Metric Analysis Skill (v12.2)

> **답변은 항상 한국어로 한다** (실행 환경이 영어로 기울어도 유지). 그림 속 라벨만 영어. 상세 §8.

> **변경 이력**
> - **v12.2 (현재)** — 환경 비의존화 + 프로그램 연동. ① `workflows/_common.md` §0 전면 개편: MCP 도구를 **접두어가 아닌 접미사로 식별**(`mcp__remote-devices__sqlcl__*` / `mcp__sqlcl__*` 양쪽), 연결 이름 하드코딩 제거 후 **목록을 읽어 매칭**(순위: changshinincaipoc > 포함 > 접속문자열 > `*_medium` > `csi*`; `csi*` 는 비번 미저장 연결 때문에 마지막), `lmes` 계열 제외. ② **접속 직후 검증 1회 필수**(`ALL_TABLES` `OWNER='OCI'` 개수 + `SYSTIMESTAMP` 타임존). ③ **지갑/TNS_ADMIN 절차 신설** — 복사해 온 지갑의 `sqlnet.ora DIRECTORY` 함정, 경로에 공백·한글 금지(ORA-17956), Windows 절차. ④ **증상별 조치표**(ORA-12154/12263/17956/28759/29024/01017/00942/03113). ⑤ `workflows/reports-pipeline.md` 신설 — 공식 11개 엑셀은 SQL 로 재현하지 말고 `ckp-reports` MCP(`ckp_make_all`/`ckp_status`)에 위임, 60초 타임아웃 오해 방지, 리포트별 필터의 진실의 출처(`balance_sql.py` REPORTS)와 마감필터 차이 명시.
