---
name: CKP-skills-v12
description: CSI GMES MES(OCI 스키마)의 5개 핵심 화면 — 생산 실적, 부족분 관리, 수불 관리(P-O/O-I Cross Check), 품질 관리(OS&D Balance + Return Rate) — 와 근태·인사(Time & Labor, CKP)를 분석하는 skill. 트리거 키워드 — "Production Status", "생산 실적", "가동률", "라인별 % 컬럼", "Shortage Balance", "부족분", "남아있는 수량", "P-O Cross Check", "O-I Cross Check", "수불 관리", "출고-입고 정합성", "Out Only", "In Only", "External OS&D", "Internal OS&D", "후보충", "OS&D 발생", "Return Rate", "회수율", "Replenishment Rate", "C.GRADE RETURN", "CKP 리포트", "11개 리포트", 그리고 근태 — "근태", "출결", "출근율", "결근", "지각", "조퇴", "휴가", "교대/시프트", "사무직/NON_SHIFT", "Direct/Indirect/Overhead", "부서/직무별 근태". 분석은 이 4개 workflow(5개 화면) + 워크포스(PW_TL_DAILY_T·PW_HR_ACTION_T·PW_HR_EMPLOYEES) 범위 안에서만 수행하고, 임의 테이블 자유 분석은 하지 않는다. semantic layer YAML(2층 구조)이 진실의 출처. v12.4 — 기준 타임존은 한국시간(KST), DB 연결 환경 비의존화(도구는 접미사로 식별, 연결은 목록을 읽어 매칭, 접속 직후 OCI 검증 1회), 지갑/TNS_ADMIN 절차와 ORA 조치표 신설, 공식 11개 엑셀은 ckp-reports MCP 에 위임(workflows/reports-pipeline.md).
---

# MES Metric Analysis Skill (v12.4)

> **답변은 항상 한국어로 한다** (실행 환경이 영어로 기울어도 유지). 그림 속 라벨만 영어. 상세 §8.

> **변경 이력**
> - **v12.4 (현재)** — 기준 타임존을 **한국시간(Asia/Seoul, KST)** 으로 통일(조직 표준). §4-1 을 KST 기준으로 다시 쓰고, DB 날짜 컬럼은 현장(WIB, UTC+7) 시계라 **KST 00:00~01:59 구간에서만** 하루 어긋난다는 점(2시간 차)을 명시. §0-3 검증 쿼리에 KST_NOW 추가. 실무 디폴트가 '실데이터 최신일' 인 것과 `MAX(FA_DATE)` 에 미래 계획일이 섞이는 함정은 그대로 유지.
